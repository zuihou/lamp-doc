...
- build: 
  ...
  - spring-framework.version>6.2.10
  - nacos.version>3.0.3
  - spring-boot-admin.version>3.5.2
  - druid.version>1.2.27
  - fastmybatis.version>3.1.7
  - commons-beanutils.version>1.11.0
  - commons-exec.version>1.5
  - commons-email.version>1.6.0
  - commons-io.version>2.20.0
  - commons-logging.version>1.3.5
- refactor:
  - 代码生成器服务，配置抽离到lamp-generator-server.yml 和 application-dev.yml
  - lamp-openapi-server、lamp-sop-admin-server配置抽离到 nacos 中
  - dubbo 注册时新增元信息(sba: ignore)，让spring-boot-admin根据元信息排除监控dubbo注册信息
  - logging.xxx 配置抽离到common.yml
- fix
  - Sop: 修复当objClass被代理后，获取不到interface BUG

### 前端

- 合并最新版vben代码
- 优化useVbenForm样式 (#6611)、操作按钮组的样式、操作按钮组显示位置、优化useVbenForm actions 布局样式
