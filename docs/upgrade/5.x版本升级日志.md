...
- spring-framework.version>6.2.17
- nacos.version>3.1.2
- spring-boot-admin.version>3.5.8
- torna-sdk.version>1.0.17
- hutool.version>5.8.44
- lombok.version>1.18.44
- druid.version>1.2.28
- mybatis-plus.version>3.5.16
- fastjson2.version>2.0.61
- guava.version>33.5.0-jre
- feat: 登录页新增忘记密码功能
- feat: 手机号+验证码登录
- refactor: 优化sop-gateway-server

## 5.8.1 2026年01月22日21:35:15
- build: 依赖升级 
  - spring-boot-starter-parent > 3.5.10
  - spring-cloud-dependencies.version>2025.0.1
  - 将okhttp版本从3.14.9升级到4.12.0
  - 将fastjson2版本从2.0.57升级到2.0.60
  - 将commons-logging版本从1.3.3升级到1.3.5
  - 将commons-io版本从2.17.0升级到2.20.0
  - 将lombok版本从1.18.36升级到1.18.42
- refactor: 重构 sop-gateway-server 和 lamp-sop-admin 的缓存功能，取消内存缓存，强制依赖redis
- refactor: 解耦 lamp-sop-admin-server，启动和运行时不在依赖 sop-gateway-server
- chore(config): 更新dubbo消费者超时及重试配置
- feat(openapi): 增加开放接口中抛出异常的演示代码和相关测试类
- fix(sop): 修正接口回调重试逻辑, 将查询条件从等于(nextTime)改为小于等于(nextTime)
- fix: 修复应用分组授权时，sop权限刷新功能不生效
- fix(dict): 修复字典缓存存储值，以及添加租户库中字典表的国际化字段
- feat(appendix): 新增根据多个业务ID和业务类型查询附件接口
- refactor(GlueFactory): 优化代码哈希计算及异常处理
- fix(sdk-core): 修复构建请求参数时的空值处理问题
- feat(MultiThreadTaskProcessor): 优化多线程任务处理器实现
