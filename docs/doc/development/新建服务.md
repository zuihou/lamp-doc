...
3. 调整配置
   ...
   将lamp-gateway-server文件的内容复制到nacos中的lamp-gateway-server.yml，然后删除resources目录的lamp-gateway-server.yml
   ...
   lamp:
     swagger:
       docket:
         dd:
           title: 测试订单服务服务
           base-package: top.tangyh.lamp.dd.controller
   #  database:
   #    initDatabasePrefix:
   #      - lamp_base
